...
class SOM(nn.Module):
    ...
    def forward(self, input):
        ...
        print("in shap: ", input.shape)
        batch_weight = self.weight.expand(batch_size, -1, -1)
        print("batch wt shap: ", batch_weight.shape)
    ...
    def self_organizing(self, input, current_iter, max_iter):
        ...
        print("bmu location shape: {}".format(bmu_locations.shape))
        ...
        print("wt shp: ", self.weight.shape)
        print("ip shp: ", input.unsqueeze(2).shape)
        print("input-wt: {}".format((input.unsqueeze(2) - self.weight).shape))
...
if __name__=="__main__":
    som = SOM(input_size=28*28*1, out_size=(16, 16))
    data = torch.randn(28,28,1)
    data = data.view(-1,28*28*1)

    for epoch in range(100):
        loss = som.self_organizing(data, epoch, 100)
        # print(summary(som,data))
        print(loss)
